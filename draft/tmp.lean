...
begin
  ...
  have had : a ≠ d,
    intro hf, rw [hf, inside_three_pt_angle] at hd,
  ...
  have : same_side_line (a'-ₗd') b' c',
    rw line_comm, refine t_shape_ray _ _ _ _ _, exact ha'd'.symm,
    rw line_comm, exact hb'c'.2.1,
    left, exact h,
  ...
  have h₂ : supplementary (∠ d a b) (∠ d a x),
    ...
    split, rintros ⟨m, hm, hdm, ham, hbm⟩,
    rw two_pt_one_line hm (line_in_lines had) ha'd' ⟨ham, hdm⟩ ⟨pt_left_in_line a' d', pt_right_in_line a' d'⟩ at hb'm,
    exact hb'c'.2.1 hb'm,
end
...
begin
  ...
  have hd''xc'' := angle_eq_same_side_unique key ha'd''x ha'd''c'' this,
...
(hbac : noncollinear b a c) (hb'a'c' : noncollinear b' a' c') 
(h₁ : ∠ b a c ≅ₐ ∠ b' a' c') (h₂ : ∠ b a d ≅ₐ ∠ b' a' d') :
...
begin
  ...
  have hxa'b' : x ∉ ↑(line a' b'),
    rw [is_between_same_side_pt, same_side_line_pt] at hb'xd',
  ...
  have hb'a'c'' : noncollinear b' a' c'',
    have h₁ : same_side_line (a'-ₗb') d' x,
      ...
      exact hxa'b',
    have h₂ : same_side_line (a'-ₗb') x c'',
      rw line_comm, refine t_shape_ray _ _ _ _ _, exact ha'b'.symm,
      rw line_comm, exact hxa'b',
      left,
    rintros ⟨l, hl, hb'l, ha'l, hc''l⟩,
    rw two_pt_one_line hm hl  at hd'm,
  have : (∠ b a c ≅ₐ ∠ b' a' c''),
    have := congr_angle_add hd hb'c'' hbac,
end

def angle_lt (α β : angle) : Prop :=
∀ a b : C.pts, β = (∠ a β.vertex b) → ∃ p : C.pts, inside_angle p (∠ a β.vertex b) ∧ ((∠ a β.vertex p) ≅ₐ α)
...
begin
  ...
  use q, split,
  refine (congr_angle_add hp _ _ _ _ _).1,
end
