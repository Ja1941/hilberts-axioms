...
begin
  ...
  rw hf at hx, exact (is_between_neq hx).2.1 rfl,
  exact (is_between_neq hx).1 hf,
  rw is_between_symm at hx, exact is_between_contra.1 ⟨hf, hx⟩,
...
begin
  ...
  cases hx with hx hx, exact absurd hx (is_between_neq hoxa).1.symm,
...
begin
  ...
  have hec : e ≠ c,
    intro hec, rw hec at hcae, exact (is_between_neq hcae).2.1 rfl,
  ...
  have hae : a ≠ e,
    intro hae, rw hae at hcae, exact (is_between_neq hcae).2.2 rfl,
...
begin
  ...
  have hee' : same_side_line (a-ₗc) e e',
    ...
    have hce' : c ≠ e',
      ...
      exact is_between_contra.1 ⟨h, h'⟩,
      cases h' with h' h', rw h' at h, exact (is_between_neq h).2.1 rfl,
      simp at h', rw h' at h, exact (is_between_neq h).2.2 rfl,
...
begin
  ...
  have hob := (is_between_neq hboc).1.symm,
