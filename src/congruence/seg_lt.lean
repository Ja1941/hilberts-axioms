...
lemma seg_lt_nontrivial {m n : seg} : (m <ₛ n) → seg_nontrivial n :=
begin
  rintros ⟨a, b, c, hn, hbac, hm⟩,
  rw [hn, seg_nontrivial_iff_neq], exact (between_neq hbac).2.1
end
...
begin
  split; intro h,
  have hab := seg_lt_nontrivial h, rw seg_nontrivial_iff_neq at hab,
  rcases h with ⟨x, a', b', haba'b', ha'xb', hm⟩,
  cases two_pt_seg_pt haba'b',
  rw [←h.1, ←h.2] at ha'xb', rw ←h.1 at hm,
  exact ⟨x, ha'xb', hm⟩,
  rcases extend_congr_seg (seg_nontrivial_iff_neq.2 (between_neq ha'xb').1) hab
    with ⟨y, haby, hy, -⟩,
...
((m <ₛ l) → (n <ₛ l)) ∧ (seg_nontrivial n → (l <ₛ m) → (l <ₛ n)) :=
begin
  ...
  rw [hnde, seg_nontrivial_iff_neq] at hn,
  rcases extend_congr_seg (seg_nontrivial_iff_neq.2 (between_neq hbac).1) hn
    with ⟨x, hdex, hbadx, -⟩,
...
begin
  ...
  replace hmn := (seg_lt_congr hn).2 (seg_nontrivial_iff_neq.2 hab.symm) hmn,
...
begin
  intro hacad,
  rcases extend_congr_seg (seg_nontrivial_iff_neq.2 (same_side_pt_neq habc).2.symm)
    (same_side_pt_neq habc).1.symm with ⟨x, habx, hacax, hu⟩,
  rw [hu c habc (seg_congr_refl _), hu d habd hacad]
end

lemma seg_tri {m n : seg} (ha'b' : seg_nontrivial m) (hab : seg_nontrivial n) :
((m <ₛ n) ∨ (m ≅ₛ n) ∨ (n <ₛ m))
∧ ¬((m <ₛ n) ∧ (m ≅ₛ n)) ∧ ¬((m <ₛ n) ∧ (n <ₛ m)) ∧ ¬(((m ≅ₛ n) ∧ (n <ₛ m))) :=
begin
  rcases seg_two_pt n with ⟨a, b, hn⟩,
  rw [hn, seg_nontrivial_iff_neq] at hab,
  ...
  have h₃ : (n <ₛ m) ↔ diff_side_pt b a c,
    ...
    replace hnm := (seg_lt_congr hm).2
      (seg_nontrivial_iff_neq.2 (same_side_pt_neq habc).2.symm) hnm,
